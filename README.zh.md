...
- 模型的提问（`ask_user_question`）同样变成卡片：该工具在每个会话 Agent 自己的层里被遮蔽，模型给的选项渲染成按钮；模型标记为 `multi_select` 时则渲染成表单，一次提交选中的整组。选项都不合适时直接回复消息即可作答。
...
- 支持多个机器人：每个飞书应用组合一行插件，额外的行用 `instance` 命名，其设置、凭据与会话彼此隔离。
...
| `instance` | — | 部署组合多个机器人时用来命名本行，使其设置、App Secret 凭据与会话 id 相互隔离。第一行不要填：未命名的行沿用原有标识。 |
| `denyTools` | `[]` | 会话 Agent 不可调用的工具，按 agent 在执行处拒绝。默认为空：人类交互类工具改为被遮蔽并在此作答，而不是拒绝。在这里写下的名字仍然优先于遮蔽。 |
| `botPeers` | `[]` | 收窄本渠道应答哪些机器人，用于让一个群里的两个 Agent 对话。留空=不收窄（与本插件其他名单一致）；每个发言的机器人都会在控制台按群各打印一次。 |
| `botHops` | `6` | 一个会话中连续由机器人触发的轮次上限，达到后停止应答；有人说话即恢复。 |
